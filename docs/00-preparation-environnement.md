...
Cette procédure prépare l'environnement logiciel utilisé par le P5 dans la distribution
WSL2 **`Ubuntu`**, Ubuntu 26.04 LTS (`resolute`).

Le projet évalué s'exécute sur AWS. La distribution WSL2 fournit le plan de contrôle CLI depuis lequel
...
[`../environment/wsl2/README.md`](../environment/wsl2/README.md).

Toutes les commandes ci-dessous sont exécutées **dans `Ubuntu` sous WSL2**. Le parcours de référence
est `inspect → prepare → status → GO TERRAFORM`.

## 1. Ouvrir la distribution WSL2
...
Le checkout doit résider sur un filesystem Linux local de la distribution WSL2.
...
- Ubuntu 26.04 / Resolute sous WSL2 ;
- la distribution WSL2 `Ubuntu` ;
- WSL2 ;
...
- autre code : écart P5 ou contrat WSL2 non respecté.
...
ne doivent pas être confondues avec le système de la distribution WSL2 de contrôle.
...
```text
1. quitter Ubuntu
2. depuis PowerShell : wsl.exe --terminate Ubuntu
3. rouvrir : wsl.exe -d Ubuntu
4. revenir dans ~/labs/p5_Openclassrooms et relancer prepare
```
...
Les opérations de cycle de vie de `Ubuntu` relèvent du dépôt `Windows_11_Pro_Custom`.
...
```text
distribution WSL  Ubuntu
OS                Ubuntu 26.04 / resolute sous WSL2
virtualisation    WSL2
