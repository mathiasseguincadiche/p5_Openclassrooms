...
# - uses : Utilise une action préexistante (ex: actions/checkout@v7).
...
> |-------|-------------|-----------------|
> | Checkout code | Récupère le code du dépôt. | `actions/checkout@v7` |
> | Setup Node.js | Installe Node.js 24. | `actions/setup-node@v7` |
...
# Image de base : Node.js 24 sur Alpine Linux
FROM node:24-alpine
...
### 4. À quoi sert `actions/checkout@v7` ?

<details>
<summary>💡 Réponse</summary>
`actions/checkout@v7` est une **action GitHub** qui **récupère le code de votre dépôt** dans le runner, afin que les étapes suivantes puissent y accéder.
</details>

### 5. À quoi sert `actions/setup-node@v7` ?

<details>
<summary>💡 Réponse</summary>
`actions/setup-node@v7` est une **action GitHub** qui **installe Node.js** (et npm/yarn) sur le runner, avec la version spécifiée.
</details>
...
| `Error: Unable to find image 'node:24-alpine'` | L'image Docker n'existe pas ou Docker Hub est inaccessible. | Vérifiez votre connexion Internet ou essayez une autre image (ex: `node:24`). |
