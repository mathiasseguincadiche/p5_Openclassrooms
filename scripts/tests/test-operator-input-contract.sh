...
p5_validate_aws_region 'us-east-1' || fail 'région AWS valide refusée'
! p5_validate_aws_region 'martinique' || fail 'région AWS invalide acceptée'
...
grep -Fq '[INCONNU] adresses des deux backends HAProxy non fournies.' \
    "$PROJECT_ROOT/scripts/tools/generer-haproxy-config.sh" \
    || fail 'générateur HAProxy : valeurs manquantes non expliquées'
grep -Fq 'Format attendu' "$PROJECT_ROOT/scripts/tools/generer-haproxy-config.sh" \
    || fail 'générateur HAProxy : format non documenté'
ok 'outil manuel HAProxy explique ses entrées'
...
grep -Fq 'INCONNU' "$PROJECT_ROOT/scripts/commands/inspect-state.sh" \
    || fail 'inspect-state ne distingue pas les valeurs inconnues'
