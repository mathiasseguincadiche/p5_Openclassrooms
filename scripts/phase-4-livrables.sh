...
copy_terraform_files() {
    ...
    TERRAFORM_SRC="terraform/exercice-1"
...
copy_ansible_playbook() {
    ...
    PLAYBOOK_SRC="ansible/playbooks/deploy.yml"
...
copy_haproxy_config() {
    ...
    HAPROXY_SRC="terraform/exercice-3/haproxy.cfg"
    ...
    if [ -f "$HAPROXY_SRC" ]; then
        cp "$HAPROXY_SRC" "$HAPROXY_DST"
        success "Fichier haproxy.cfg copié vers $HAPROXY_DST"
    else
        # Essayer d'autres emplacements
        if [ -f "scripts/$HAPROXY_SRC" ]; then
            cp "scripts/$HAPROXY_SRC" "$HAPROXY_DST"
            success "Fichier haproxy.cfg copié depuis scripts/"
        else
            error "Fichier haproxy.cfg introuvable"
            exit 1
        fi
    fi
}
...
| Heure      | Action                                      | Commande                          | Résultat                     |
|------------|---------------------------------------------|-----------------------------------|------------------------------|
| 10:00-11:00 | Configuration de l'environnement            | `sudo apt update && sudo apt upgrade -y` | ✅ Succès                     |
| 11:00-12:00 | Installation des outils                     | `sudo apt install -y terraform ansible awscli` | ✅ Succès                     |
| 14:00-18:00 | Exercice 1 : Terraform + Ansible + Angular | `terraform apply -auto-approve` | ✅ 2 VMs déployées            |
