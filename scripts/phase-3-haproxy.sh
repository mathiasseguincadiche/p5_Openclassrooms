...
PHASE_DIR="04_EXERCICES/03_HAPROXY"
TERRAFORM_DIR="terraform/exercice-3"
SCRIPTS_DIR="scripts"
...
get_backend_ips() {
    ...
    info "Récupération des IPs privées des VMs backend..."
    
    # Essayer de récupérer les IPs depuis les outputs Terraform
    BACKEND_1_IP=$(terraform output -raw backend_1_private_ip 2>/dev/null)
    BACKEND_2_IP=$(terraform output -raw backend_2_private_ip 2>/dev/null)
    
    # Si les outputs ne sont pas définis, essayer de les récupérer depuis les instances
    if [ -z "$BACKEND_1_IP" ] || [ -z "$BACKEND_2_IP" ]; then
        info "Récupération des IPs depuis les instances EC2..."
        BACKEND_1_IP=$(aws ec2 describe-instances --query "Reservations[0].Instances[0].PrivateIpAddress" --output text 2>/dev/null)
        BACKEND_2_IP=$(aws ec2 describe-instances --query "Reservations[0].Instances[1].PrivateIpAddress" --output text 2>/dev/null)
    fi
    ...
    info "  Backend 1 : $BACKEND_1_IP"
    info "  Backend 2 : $BACKEND_2_IP"
    
    # Sauvegarder les IPs
    echo "$BACKEND_1_IP" > /tmp/backend_1_ip.txt
    echo "$BACKEND_2_IP" > /tmp/backend_2_ip.txt
}
...
generate_haproxy_config() {
    ...
    BACKEND_1_IP=$(cat /tmp/backend_1_ip.txt)
    BACKEND_2_IP=$(cat /tmp/backend_2_ip.txt)
    ...
    if [ -f "$SCRIPTS_DIR/generer-haproxy-config.sh" ]; then
        if bash "$SCRIPTS_DIR/generer-haproxy-config.sh" "$BACKEND_1_IP" "$BACKEND_2_IP"; then
            success "Configuration HAProxy générée"
            info "Contenu du fichier haproxy.cfg :"
            cat "$HAPROXY_CONFIG"
        else
...
deploy_haproxy_config() {
    ...
    if scp -i p5-key.pem -o StrictHostKeyChecking=no "$HAPROXY_CONFIG" ubuntu@"$HAPROXY_IP":/tmp/; then
    ...
    if ssh -i p5-key.pem -o StrictHostKeyChecking=no ubuntu@"$HAPROXY_IP" \
        "sudo cp /tmp/$HAPROXY_CONFIG /etc/haproxy/haproxy.cfg && \
...
test_fault_tolerance() {
    ...
    if ssh -i p5-key.pem -o StrictHostKeyChecking=no ubuntu@"$BACKEND_1_IP" \
