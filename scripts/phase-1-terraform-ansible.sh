...
# DESCRIPTION : Phase 1 - Déploiement avec Terraform + Ansible + Angular
...
PHASE_DIR="04_EXERCICES/01_TERRAFORM_ANSIBLE"
TERRAFORM_DIR="terraform/exercice-1"
ANSIBLE_DIR="ansible"
HOSTS_FILE="hosts_aws"
...
show_help() {
    ...
    echo ""
    echo "Ce script déploie 2 VMs AWS avec NGINX + Application Angular."
    echo ""
    echo "Options :"
...
get_vm_ips() {
    ...
    VM1_IP=$(terraform output -raw nginx_1_public_ip 2>/dev/null)
    VM2_IP=$(terraform output -raw nginx_2_public_ip 2>/dev/null)
...
create_ansible_inventory() {
    ...
    cat > "$HOSTS_FILE" <<EOF
[webservers]
${VM1_IP} ansible_user=ubuntu ansible_ssh_private_key_file=p5-key.pem
${VM2_IP} ansible_user=ubuntu ansible_ssh_private_key_file=p5-key.pem
EOF
...
test_ansible_connection() {
    ...
    if ansible all -i "$HOSTS_FILE" -m ping; then
        ...
        info "Vérifiez :"
        info "  1. La clé SSH p5-key.pem existe"
...
verify_deployment() {
    ...
    if curl -s -I "http://$VM1_IP" | grep -q "HTTP/1.1 200 OK"; then
    ...
    if curl -s -I "http://$VM2_IP" | grep -q "HTTP/1.1 200 OK"; then
        check "NGINX répond sur $VM2_IP"
    else
        warning "NGINX ne répond pas sur $VM2_IP"
    fi
    
    # Vérifier que l'application Angular est servie
    info "Vérification que l'application Angular est servie..."
    if curl -s "http://$VM1_IP" | grep -q "angular"; then
        check "Application Angular servie sur $VM1_IP"
    else
        warning "L'application Angular n'est pas servie sur $VM1_IP (page par défaut NGINX ?)"
    fi
    
    if curl -s "http://$VM2_IP" | grep -q "angular"; then
        check "Application Angular servie sur $VM2_IP"
    else
        warning "L'application Angular n'est pas servie sur $VM2_IP (page par défaut NGINX ?)"
    fi
...
info "Objectif : Déployer 2 VMs AWS avec NGINX + Application Angular"
echo ""
...
success "2 VMs AWS déployées avec NGINX + Application Angular !"
