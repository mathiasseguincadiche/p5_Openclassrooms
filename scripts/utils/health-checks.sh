...
check_phase_0() {
 ...
 if [ -d "/workspace/mathiasseguincadiche__p5_Openclassrooms" ]; then
...
check_phase_2() {
 ...
 if [ -d "docs/exercices/data" ]; then
 check "Dossier docs/exercices/data existe"
 log_info "Dossier data existe"
 
 if [ -f "docs/exercices/data/nginx-access.log" ]; then
 check "Fichier nginx-access.log existe"
 log_info "Fichier nginx-access.log existe"
 else
 warning "Fichier nginx-access.log introuvable (un exemple sera créé)"
 log_warning "Fichier nginx-access.log introuvable"
 fi
 else
 warning "Dossier docs/exercices/data introuvable"
 ...
 if curl -k -I "$endpoint" | grep -q "HTTP/1.1 200 OK"; then
 ...
 if curl -k -s -X GET "$endpoint/_cat/indices?v" | grep -q "nginx-access"; then
 check "Index nginx-access existe"
 log_info "Index nginx-access existe"
 
 local doc_count=$(curl -k -s -X GET "$endpoint/nginx-access-*/_count" | jq -r '.count // "0"' 2>/dev/null)
...
check_phase_3() {
 ...
 for file in "main.tf" "variables.tf" "outputs.tf" "haproxy.cfg"; do
 ...
 if curl -k -I "$haproxy_url" | grep -q "HTTP/1.1 200 OK"; then
