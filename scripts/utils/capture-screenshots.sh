...
check_dashboard_exists() {
 ...
 local start_time=$(date +%s)
 while true; do
 local dashboards=$(curl -k -s -X GET "${KIBANA_URL%/_dashboards}/_dashboards/api/saved_objects/dashboard" 2>/dev/null)
...
generate_exercice_2_captures_headless() {
 ...
 local dashboards_json=$(curl -k -s -X GET "${KIBANA_URL%/_dashboards}/_dashboards/api/saved_objects/dashboard" 2>/dev/null)
