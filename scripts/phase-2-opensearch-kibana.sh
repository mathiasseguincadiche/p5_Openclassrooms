...
PHASE_DIR="04_EXERCICES/02_OPENSEARCH"
TERRAFORM_DIR="terraform/exercice-2"
DATA_DIR="docs/exercices/data"
...
check_phase_2_prerequisites() {
    ...
    check_file_exists "$DATA_DIR/nginx-access.log" || {
        warning "Le fichier $DATA_DIR/nginx-access.log n'existe pas"
    ...
    while true; do
        ...
        case "$status" in
            "Active")
                ...
                ;;
            "Processing")
                info "Le cluster est en cours de création (statut: $status). Attente de 30 secondes..."
                sleep 30
                ;;
            "Failed")
                error "Le cluster OpenSearch a échoué (statut: $status)"
                exit 1
                ;;
            *)
                info "Statut inconnu : $status. Attente de 30 secondes..."
                sleep 30
                ;;
        esac
    done
...
# Configure l'access policy pour Kibana
configure_access_policy() {
    step 6 "Configuration de l'access policy pour Kibana"
    
    info "Récupération de votre IP publique..."
    MY_IP=$(curl -s ifconfig.me)
    info "Votre IP publique : $MY_IP"
    
    info "Création du fichier access-policy.json..."
    cat > /tmp/access-policy.json <<EOF
{
  ...
  "Statement": [{
    ...
    "Principal": {"AWS": "*"},
    "Action": ["es:*"],
    "Resource": "arn:aws:es:us-east-1:$(aws sts get-caller-identity --query Account --output text):domain/$DOMAIN_NAME/*",
    "Condition": {"IpAddress": {"aws:SourceIp": ["$MY_IP/32"]}}
  }]
}
EOF
    
    info "Application de l'access policy..."
    if aws es update-domain-config --domain-name "$DOMAIN_NAME" --access-policies file:///tmp/access-policy.json; then
        success "Access policy configurée avec succès"
        info "Attente de 1 minute pour que la policy soit appliquée..."
        sleep 60
    else
        error "Échec de la configuration de l'access policy"
        exit 1
    fi
}
...
test_kibana_access() {
    ...
    if curl -k -I "$KIBANA_URL" | grep -q "HTTP/1.1 200 OK"; then
...
load_nginx_logs() {
    ...
    LOGS_FILE="$DATA_DIR/nginx-access.log"
    if [ ! -f "$LOGS_FILE" ]; then
...
EOF
    ...
    while IFS= read -r line; do
        ...
        if ! curl -k -s -X POST "$OPENSEARCH_ENDPOINT/nginx-access-$(date +%Y.%m.%d)/_doc" \
...
verify_logs_loaded() {
    ...
    if curl -k -s -X GET "$OPENSEARCH_ENDPOINT/_cat/indices?v" | grep -q "nginx-access"; then
